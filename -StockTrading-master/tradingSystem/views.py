...
def stock_info(request, stock_id):
    ...
    choosenStock = models.StockInfo.objects.filter(stock_id=stock_id)  # 获取stock_info中的数据
    ...
    if (choosenStock[0].stock_type == "上证"):
        ...
        sql = "SELECT * FROM `%s`"        #   修改代码 2023.04.02 查表
 ...
 # # #
        if (f):
            ...
            f = 0
        # hold_vol = getAstock.getAstock(stock_id + ".SZ")   修改代码2023.03.29 接口没有权限
    cursor.close()
    conn.close()
    ...
    return render(request, 'stock_details.html', context)  # 跳转，请求对象，模板名称，和传递的数据。
...
def out(request, stock_id):
    ...
    if (choosenStock[0].stock_type == "上证"):
        ...
        if (f):
            ...
            f = 0
        #hold_vol = getAstock.getAstock(stock_id + ".SZ")  #修改代码 20230402 21：21 没有权限
    cursor.close()
    conn.close()
