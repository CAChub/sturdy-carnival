...
def adm_stock_info(request, stock_id):
    ...
    if (choosenStock[0].stock_type == "上证"):
        # hold_vol = getAstock.getAstock(stock_id + ".SH")  # 修改代码 2023.03.29 获取高管信息Tushaer没权限
        hisData = getHistoryData.getHistoryData(stock_id + ".SH")  # 获取历史行情数据
    else:
        # hold_vol = getAstock.getAstock(stock_id + ".SZ")  # 修改代码 2023.03.29 获取高管信息没权限
        hisData = getHistoryData.getHistoryData(stock_id + ".SZ")  # 获取历史行情数据
    stock = StockInfo.objects.get(stock_id=stock_id)  # 重定向
    comments = StockComment.objects.filter(stock_id=stock)  # 重定向
    #  打包数据成字典
    context = {
    ...
    # Django 中的一个视图函数，用于将数据渲染到指定的模板中，并以 HTML 页面的形式返回给浏览器
